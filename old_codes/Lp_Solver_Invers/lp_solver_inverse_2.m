...
lambda_range = logspace(-2, 1.4, 10);
...
for i = 1:num_lambda
    ...
    x_lambda_case1 = Lp_solver_depthver2(sens_norm, b_ideal_normalized, Recon.p, Recon.lambda_iter, ...
    ...
    res1 = norm(sens_norm * x_lambda_case1 - b_ideal_normalized, 2);
...
% Ortak metrikler için y ekseni sınırları (ilk üç metrik)
all_metrics = [nssd_array; nsad_array; nr_array];
common_ylim = [min(all_metrics)*0.95, max(all_metrics)*1.05];

% mm cinsinden Volume ve Centroid Error için y ekseni sınırları
...
% Voxel cinsinden Volume ve Centroid Error için y ekseni sınırları
...
('recon_data.mat', 'recon_data.xlsx');
