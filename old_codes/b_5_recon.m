...
cfg.unitinmm = 0.1;
[m, n] = size(A);
delta_target = noise_rms * sqrt(m);  % Makale Eq.21
%% Lambda Aralığı (Genişletilmiş)
lambda_range = recon.lambda_range;
num_lambda = length(lambda_range);
iso_value = recon.iso_value ;
...
for i = 1:num_lambda
    ...
    % Önceden hesaplanmış çözümleri yükle
    x_lambda_case1 = recon.x_lambda_case1{i};
    x_lambda_case2 = recon.x_lambda_case2{i};
    
    % Thresholding işlemleri
    ...
    % Morozov
    delta_est_case2(i) = noise_rms * sqrt(df_smooth2);
