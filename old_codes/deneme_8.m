...
source_pos      = [137, 245];   % Kaynak (x,y)
scanX           = 30;           % Tarama noktaları X yönü
scanY           = 30;           % Tarama noktaları Y yönü
...
detector_spacing = 10;
%detector_radius  = input('Dedektör yarıçapı giriniz (örn: 1): ');
detector_radius  = 2;
...
image_folder = "C:\Users\TUSEB\Desktop\MCXStudio-win64-v2023\MCXStudio\MATLAB\mcxlab\enes\Analiz\loop_2_analiz";
files = dir(fullfile(image_folder,'pos_*.png'));

% Dosya isimlerinden koordinatları çıkar
coords = zeros(numel(files),2);
for k = 1:numel(files)
    tok         = regexp(files(k).name, 'pos_(\d+)x(\d+)\.png', 'tokens');
    coords(k,1) = str2double(tok{1}{1});  % X
    coords(k,2) = str2double(tok{1}{2});  % Y
end

% Önce X sonra Y’ye göre sırala
[~, order] = sortrows(coords, [1 2]);    % önce 1. sütun (X), sonra 2. sütun (Y)
files  = files(order);
coords = coords(order,:);                % mutlaka eşleştir

% Sıralı files ve coords üzerinden image_grid’i doldur
image_grid = cell(scanX, scanY);
for idx = 1:numel(files)
    x = coords(idx,1);
    y = coords(idx,2);

    path = fullfile(image_folder, files(idx).name);
    info = imfinfo(path);
    A    = imread(path);
