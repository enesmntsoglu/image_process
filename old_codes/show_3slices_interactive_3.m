function show_3slices_interactive_3(volume3D, vol_preX, methodName)
% SHOW_3SLICES_INTERACTIVE_2
%   volume3D   : 3B reconstructed hacim (örneğin, Reconstructed_Triangle)
%   vol_preX   : Orijinal phantom hacmi (vol_preX)
%   methodName : Görselleştirme başlığında görünecek isim/etiket
%
% Bu fonksiyon, interaktif slice (XY, YZ, XZ) ve isosurface görselleştirmesini,
% ayrıca bir popup menü aracılığıyla hangi hacmin görüntüleneceğini seçme imkanı sunar.

    % Hacim boyutlarını al (iki hacmin boyutlarının aynı olduğunu varsayıyoruz)
    [Nx, Ny, Nz] = size(volume3D);
    
    % Başlangıçta görüntülenecek hacmi belirleyelim (default: reconstructed)
    currentVolume = volume3D;

    % Meshgrid oluştur (her iki hacim için aynı boyutlar)
    ...
    hXY = imagesc(currentVolume(:,:,z_init), 'Parent', hAx1);
    ...
    hYZ = imagesc(squeeze(currentVolume(x_init,:,:)), 'Parent', hAx2);
    ...
    hXZ = imagesc(squeeze(currentVolume(:,y_init,:)), 'Parent', hAx3);
    ...
    % Iso değeri: örnek olarak, maksimumun yarısı
    iso_val = 0.5 * max(currentVolume(:));
    isoStruct = isosurface(X, Y, Z, currentVolume, iso_val);
    % IsoSurface için axes oluştur
    hIsoAx = axes('Position',[0.05 0.1 0.90 0.35]);
    ...
    % Orijinal vertices bilgisini sakla (rotasyon için referans)
    origVertices = isoStruct.vertices;
    ...
    %% Volume Seçim Popup'ı
    % Kullanıcı, reconstructed hacim ile vol_preX arasında seçim yapabilsin
    popupVol = uicontrol('Style','popupmenu',...
        'String',{'Reconstructed Volume','Original Phantom (vol_preX)'},...
        'Units','normalized','Position',[0.8 0.92 0.15 0.05],...
        'Callback',@(src,evt) updateVolumeSelection);

    %% Callback Fonksiyonları

    % Seçilen hacme göre slice'ları güncelle
    function updateSlices
        ...
        % XY dilimi
        set(hXY, 'CData', currentVolume(:,:,zVal));
        title(hAx1, sprintf('XY Slice (Z = %d)', zVal), 'FontSize',12);
        
        % YZ dilimi
        set(hYZ, 'CData', squeeze(currentVolume(xVal,:,:)));
        title(hAx2, sprintf('YZ Slice (X = %d)', xVal), 'FontSize',12);
        
        % XZ dilimi
        set(hXZ, 'CData', squeeze(currentVolume(:,yVal,:)));
        title(hAx3, sprintf('XZ Slice (Y = %d)', yVal), 'FontSize',12);
    end

    % IsoSurface güncelleme: Seçilen hacme göre yeniden hesaplanır
    function updateIsoSurface
        iso_val = 0.5 * max(currentVolume(:));
        isoStruct = isosurface(X, Y, Z, currentVolume, iso_val);
        set(hIso, 'Faces', isoStruct.faces, 'Vertices', isoStruct.vertices);
        origVertices = isoStruct.vertices;
        title(hIsoAx, sprintf('IsoSurface (Iso = %.2f)', iso_val), 'FontSize',12);
    end

    % IsoSurface rotasyonu: Slider'lardan gelen açı değerleriyle
    ...
    % Popup değiştiğinde, currentVolume güncellenir ve slice ile isosurface yeniden hesaplanır.
    function updateVolumeSelection
        val = popupVol.Value;
        if val == 1
            currentVolume = volume3D;
        else
            currentVolume = vol_preX;
        end
        updateSlices();
        updateIsoSurface();
    end
