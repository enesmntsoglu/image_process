...
%% db
signal_max = max(abs(b_ideal_normalized(:)));
noise_level_dB = 20 * log10(noise_std / signal_max);

signal_mean = mean(b_ideal_normalized(:));
noise = b_case2 - b_ideal_normalized;
noise_mean = mean(abs(noise(:)));
SNR_dB = 20 * log10(signal_mean / noise_mean);
...
lambda_range = logspace(-2, 1.7, 1000); 
...
loglog(reg_norm_case2(idx_LC_case2), res_norm_case2(idx_LC_case2), 'ys', 'MarkerSize',10, 'LineWidth',2);
...
threshold_LC       = 0.1 * max(Reconstructed_LC(:));
...
threshold_UC       = 0.1 * max(Reconstructed_UC(:));  

% 2x4 grid şeklinde subplot kullanarak tüm rekonstrüksiyonları görselleştiriyoruz.
...
title('LC Rekonstrüksiyon'); view(3); axis tight; camlight;
...
subplot(2,4,7);
patch(isosurface(Reconstructed_UC, threshold_UC), 'FaceColor','cyan', 'EdgeColor','none');  % 'k' ile siyah renk seçildi
title('U-Curve Rekonstrüksiyon'); view(3); axis tight; camlight;
...
% Tek bir pencere içerisinde 3 grafik (NSSD, NSAD, NR) oluştur
figure('Name','Hata Metrikleri Karşılaştırması');

subplot(1,3,1);
bar(nssd_array);
set(gca, 'XTick', 1:num_methods, 'XTickLabel', methods);
