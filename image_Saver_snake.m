...
baseOutputFolder = 'C:\Users\TUSEB\Desktop\MCXStudio-win64-v2023\MCXStudio\MATLAB\mcxlab\enes\Image_Saver\Images\deneme\';
...
for scanIndex = 1:numScans
    ...
    for x = 1:grid_size(1)     % <-- dış döngü artık X

    % Y ekseni sırası: tek satır normal, çift satır ters
    if mod(x, 2) == 1
    ...
    for y_idx = 1:length(y_range)
        y = y_range(y_idx);   % <-- iç döngü artık Y

        try
            disp(['Pozisyon: ', num2str(x), 'x', num2str(y), ...
                  ' için tetikleme bekleniyor...']);
    ...
    % İki tarama arasında kamerayı düzgün sıfırla
    stop(vid);
    
    % Kameranın tamamen durmasını bekle
    while isrunning(vid)
        pause(0.1);
    end
    ...
    % Kamera stabilizasyonu için bekleme
    pause(0.5);
end
