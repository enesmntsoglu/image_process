...
params.nu = 1e-4;                
params.eps0 = 1e-10;             
params.rootfact = 0.65;           
params.iso_ratio = 0.3;          

% --- Lambda iteration (Tam Yakınsama) ---
params.lambda_init = 1;          
params.lambda_min = 1e-4;        
params.lambda_max = 100;        % Lambda limitini serbest bıraktık.
params.lambda_damping = 1;  %0.5   
params.lambda_rel_tol = 1e-3;    % (Eski: 1e-3) Lambda'nın virgülden sonraki 5. hanesi bile oturana kadar pes etmez.
params.outer_max_iter = 15;     % (Eski: 50) Sabaha kadar vakti var, tam sonuca ulaşması için limit 200'e çekildi.

% --- Trace estimation (Yüksek İstatistiksel Doğruluk) ---
params.trace_samples = 32;       % (Eski: 4) Hutchinson tahminindeki istatistiksel gürültüyü devasa oranda bastırır.
params.trace_tol = 1e-4;         % (Eski: 1e-2) Rastgele vektörler bile hedefe tam oturana kadar çözülür.
params.trace_maxit = 100;       % PCG'ye hedefe varması için çok geniş bir limit verildi.
...
if params.use_calibration
    ...
    x_coarse_g = Lp_solver_depthver2_gpu_2_double_v3_fast( ...
        ...
        params.max_itr_cal, params.nu, params.eps0, x_cal_init, Dg, false, 50);
...
for z_idx = 1:nz
    ...
    % V_raw(:,:,z) = [X, Y] → transpose ile imagesc'e ver: rows=Y, cols=X
    slice_data = squeeze(V_raw(:,:,z_idx))';
    imagesc(x_mm, y_mm, slice_data, clim_raw);
    set(gca, 'YDir', 'normal');
    axis image;
    colormap(gca, 'hot');
    ...
    % Sol sutun: Y ekseni etiketi
    if mod(z_idx-1, ncols) == 0
        ylabel('Y (mm)', 'FontSize', 7);
    else
        set(gca, 'YTickLabel', []);
    end
    % Alt satir: X ekseni etiketi
...
for z_idx = 1:nz
    ...
    imagesc(x_mm, y_mm, slice_data, clim_thr);
    set(gca, 'YDir', 'normal');
    axis image;
    colormap(gca, 'hot');
