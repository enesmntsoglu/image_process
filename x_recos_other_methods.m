...
eps0       = 1e-10;
rootfact   = 0.50;          % Derinlik ağırlıklandırma üssü (MPD v7 ile tutarlı)
...
lambda_range = 1;%logspace(-3, 2.5, 15);
...
if have_global
    ...
    if offx < 0 || offy < 0 || offx > (scanX - vx) || offy > (scanY - vx)
        ...
        % KONVANSIYON: dim1 = X = scanX (offx ile), dim2 = Y = scanY (offy ile).
        % Reconstructed = [X(dim1), Y(dim2), Z] oldugundan transpoze YOK.
        Vfull_LC       = zeros(scanX, scanY, nt);
        Vfull_GCV      = zeros(scanX, scanY, nt);
        Vfull_Morozov  = zeros(scanX, scanY, nt);
        Vfull_Triangle = zeros(scanX, scanY, nt);
        Vfull_Corner   = zeros(scanX, scanY, nt);
        Vfull_UC       = zeros(scanX, scanY, nt);

        Vfull_LC(offx+(1:vx), offy+(1:vx), :)       = Reconstructed_LC;
        Vfull_GCV(offx+(1:vx), offy+(1:vx), :)      = Reconstructed_GCV;
        Vfull_Morozov(offx+(1:vx), offy+(1:vx), :)  = Reconstructed_Morozov;
        Vfull_Triangle(offx+(1:vx), offy+(1:vx), :) = Reconstructed_Triangle;
        Vfull_Corner(offx+(1:vx), offy+(1:vx), :)   = Reconstructed_Corner;
        Vfull_UC(offx+(1:vx), offy+(1:vx), :)       = Reconstructed_UC;
    end
end
...
iso_ratio = 0.45;
[Nx, Ny, Nz] = size(Reconstructed_LC);   % dim1=X(satır), dim2=Y(sütun), dim3=Z
...
if have_global
    ...
    [NxG, NyG, NzG] = size(Vfull_LC);   % dim1=X=scanX, dim2=Y=scanY, dim3=Z
...
function drawIsoSurface(ax, V, iso_ratio, ttl, fcolor, Nx, Ny, Nz)
    ...
    % isosurface: Vert(:,1)=dim2(Y), Vert(:,2)=dim1(X). Konvansiyon: gfx-x ekseni = X(dim1).
    Vert = Vert(:, [2 1 3]);   % -> Vert(:,1)=X(satır), Vert(:,2)=Y(sütun)
    ...
    xlabel(ax,'X (satır)'); ylabel(ax,'Y (sütun)'); zlabel(ax,'Z');
    title(ax, ttl);
